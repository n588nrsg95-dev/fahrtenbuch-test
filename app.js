...
function askMonth(){const months=availableMonths();if(!months.length){alert('Es sind noch keine Einträge vorhanden.');return null}if(months.length===1)return months[0];const lines=months.map((m,i)=>`${i+1}: ${monthLabel(m)}`).join('\n');const answer=prompt('Für welchen Monat soll die Abrechnung erstellt werden?\n\n'+lines+'\n\nBitte Nummer eingeben:',String(months.length));if(answer===null)return null;const idx=Number(String(answer).trim())-1;if(!Number.isInteger(idx)||idx<0||idx>=months.length){alert('Ungültige Auswahl. Bitte Export erneut starten.');return null}return months[idx]}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('rateInput').onchange=()=>{state.rate=parseRate($('rateInput').value);save()}; $('excelBtn').onclick=exportExcel; $('pdfBtn').onclick=buildPdf; $('pdfBack').onclick=()=>show('settings'); $('printPdf').onclick=()=>window.print();
